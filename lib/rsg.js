...
function RSG (input, opts) {
  ...
  opts.baseStyle = opts.baseStyle === undefined ? true : !!opts.baseStyle
...
RSG.prototype.getWebpackConfig = function () {
  ...
  // include base styles from CSS imports if enabled (default), otherwise skip them
  var cssExtractor
  var cssLoader
  if (this.opts.baseStyle) {
    cssExtractor = new ExtractTextPlugin('bundle.css')
    cssLoader = cssExtractor.extract('css-loader!cssnext-loader')
  } else {
  ...
  return deepmerge({
    ...
    ] : []).concat(self.opts.baseStyle ? cssExtractor : []),
    module: {
      loaders: loaders
    }
